...
def phone_full_report(phone_input):
    ...
    try:
        ...
        if resp.status_code == 200 and "If you have Telegram" in resp.text:
            data["📱 Telegram"] = {"✅ Аккаунт существует": tg_url}
        else:
            data["📱 Telegram"] = {"❌ Не найден": "Скрыт или не зарегистрирован"}
    except:
        data["📱 Telegram"] = {"⚠️ Ошибка проверки": "Сервис недоступен"}
...
def handle_text(message):
    ...
    bot.reply_to(message, "🐸 Не распознано. Отправьте: номер, ФИО, email, VIN, IP, домен, BTC/ETH")
